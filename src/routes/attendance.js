...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      const today = istDateStr();
      const existing = await AttendanceRecord.findOne({ emp_id: req.user.id, date: today });

// Allow new check-in only if previous record was rejected
if (existing) {

  // If leave or attendance rejected → remove old record
  if (existing.status === "Rejected" || existing.leave_status === "Rejected") {
    ...
    return res.status(409).json({
      ...
      if (existing && existing.leave_status !== "Rejected") {
        return res.status(409).json({
          success: false,
          message: "Attendance already recorded for today",
        });
      ...
      if (dutyType === "On Duty" && !sector)
        return res
          .status(400)
          .json({ success: false, message: "Sector is required for On Duty" });
      ...
      const checkinTime =
        capturedAt && timeRe.test(capturedAt) ? capturedAt : istTimeStr();
      const checkinDate =
        capturedDate && dateRe.test(capturedDate) ? capturedDate : today;
...
router.post(
  ...
  [
    ...
    body("leaveType")
      .isIn(["Sick Leave", "Casual Leave", "Half Day", "Emergency Leave"])
    ...
    try {
      ...
      if (startD < minDate)
        return res
          ...
          .json({
            success: false,
            message: "Leave cannot be applied more than 30 days in the past",
          });
      if (endD > maxDate)
        return res
          .status(400)
          .json({
            success: false,
            message: "Leave can only be planned up to 10 days in advance",
          });
      ...
      if (existing) {
        return res
          ...
          .json({
            success: false,
            message: `A record already exists for ${date}. Please choose a different start date.`,
          });
      }
...
router.put(
  ...
  authenticate,
  authorize("employee"),
  upload.single("checkoutSelfie"),
  async (req, res) => {
    try {
      const isEmergency =
        req.body.emergency === "true" || req.body.emergency === true;
      ...
      if (record.status !== "Draft")
        return res
          ...
          .json({
            success: false,
            message: "Cannot checkout - record already submitted",
          });
      ...
      const checkinDateTime = new Date(
        `${record.date}T${record.checkin_time}:00+05:30`,
      );
      ...
      const effectiveNow =
        capturedAtBody && timeReCheck.test(capturedAtBody)
          ? (() => {
              const d = new Date(`${record.date}T${capturedAtBody}:00+05:30`);
              return d <= now ? d : now;
            })()
          : now;
      ...
      if (capturedAt && timeRe.test(capturedAt)) {
        ...
        if (capturedDT <= now) {
          ...
          workedHours =
            Math.round(((capturedDT - checkinDateTime) / 3600000) * 100) / 100;
        }
      }
      ...
      if (record.manager_id) {
        ...
        await notify(
          ...
          `${emp.name}'s attendance for ${record.date} requires your approval`,
          "warning",
          record._id,
          "/manager/queue",
        );
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      if (req.user.role === "manager") {
        ...
        if (!["Pending", "Rejected"].includes(record.status))
          return res
            ...
            .json({
              success: false,
              message: "Record cannot be approved in current state",
            });
      }
      ...
      const notifTitle = record.leave_type
        ? `Leave Approved ✓`
        : "Attendance Approved ✓";
...
router.put(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const rejectTitle = record.leave_type
        ? `Leave Rejected ✗`
        : "Attendance Rejected ✗";
    ...
    try {
      ...
      if (record.status !== "Rejected")
        return res
          ...
          .json({
            success: false,
            message: "HR override only allowed on Rejected records",
          });
    ...
    try {
      ...
      if (!record.checkout_time)
        return res
          ...
          .json({
            success: false,
            message: "Must checkout before requesting leave",
          });
      if (record.leave_type)
        return res
          ...
          .json({
            success: false,
            message: "Leave already requested for this record",
          });
      ...
      if (record.manager_id) {
        ...
        await notify(
          record.manager_id,
          `${leaveType} Request`,
          `${emp.name} has requested ${leaveType} for ${record.date}: ${reason}`,
          "warning",
          record._id,
          "/manager/queue",
        );
        ...
        // Email manager
        const manager = await User.findById(record.manager_id)
          .select("email name")
          .lean();
        if (manager?.email) {
          await sendMail(
    ...
    const result = await AttendanceRecord.aggregate([
      { $match: matchFilter },
      {
        $group: {
          ...
          on_duty: {
            $sum: { $cond: [{ $eq: ["$duty_type", "On Duty"] }, 1, 0] },
          },
          office_duty: {
            $sum: { $cond: [{ $eq: ["$duty_type", "Office Duty"] }, 1, 0] },
          },
          sick_leave: {
            $sum: { $cond: [{ $eq: ["$leave_type", "Sick Leave"] }, 1, 0] },
          },
          casual_leave: {
            $sum: { $cond: [{ $eq: ["$leave_type", "Casual Leave"] }, 1, 0] },
          },
          half_day: {
            $sum: { $cond: [{ $eq: ["$leave_type", "Half Day"] }, 1, 0] },
          },
          emergency_leave: {
            $sum: {
              $cond: [{ $eq: ["$leave_type", "Emergency Leave"] }, 1, 0],
            },
          },
          total_leaves: {
            $sum: { $cond: [{ $ne: ["$leave_type", null] }, 1, 0] },
          },
          lop_count: {
            $sum: { $cond: [{ $eq: ["$status", "Rejected"] }, 1, 0] },
          },
    ...
    try {
      ...
      if (record.status !== "Rejected")
        return res
          ...
          .json({
            success: false,
            message: "Only rejected records can be re-applied",
          });
      ...
      if (record.manager_id) {
        ...
        await notify(
          ...
          `${emp.name} has re-submitted attendance for ${record.date} after rejection. Reason: ${reason}`,
          "info",
          record._id,
          "/manager/queue",
        );
